...
class Animator:
    def __init__(self,name, size, delta_time):
        self.name = name
        self.size = size
        ...
        self.total_frames = self.frames_size[self.animation_name]
    
    def update(self, dt):
        self.actual_animation_time -= dt
        if self.actual_animation_time >= self.animation_delta:
            self.actual_animation_time = 0
    ...
    def animate(self, x, y, screen, flip=False, color=None):
        animation_name = self.animation_name + "_" + str(int((self.actual_animation_time/self.animation_delta)) % self.total_frames)
